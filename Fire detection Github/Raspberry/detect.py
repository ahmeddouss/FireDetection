...
def run(model: str, camera_id: int, width: int, height: int, num_threads: int,enable_edgetpu: bool) -> None:
  ...
  parser.add_argument("--model", default="fire.tflite", help="path to model file")
  ...
  while cap.isOpened():
    ...
    # Run object detection estimation using the .
    detection_result = detector.detect(input_tensor)

    # Draw keypoints and edges on input image
    image = utils.visualize(image, detection_result)
    
    # Extracting labes from us 
    detection = us.visualize(detection_result)
    

    #Sending that labes via Arduino
    sdata = serial.Serial('/dev/ttyUSB0',9600, timeout=1)
    sdata.reset_input_buffer()
    print("Sending Camera fire detection : "+detection+"%")
    sdata.write(detection.encode('utf-8'))
    time.sleep(0.3)
